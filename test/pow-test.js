...
tape("pow.invert(y) coerces range values to numbers", function(test) {
  ...
  test.equal(scale.scalePow().range([new NanoDate(1990, 0, 1), new NanoDate(1991, 0, 1)]).invert(new NanoDate(1990, 6, 2, 13)), 0.5);
  test.end();
});
...
tape("pow.domain(domain) coerces domain values to numbers", function(test) {
  test.deepEqual(scale.scalePow().domain([new NanoDate(1990, 0, 1), new NanoDate(1991, 0, 1)]).domain(), [631180800000, 662716800000]);
